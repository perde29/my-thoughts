...
class AddUserCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Add User Command Wizad');
        ...
        $successMessage = sprintf('%s was successfuly created: %s',
            $isAdmin ? 'Administrator use' : 'User' , $email
            );

        $io->success($successMessage);

        $event = $stopwatch->stop('add-user-command');
        $stopwatchMessage = sprintf('New user\'s id: %s / Elapsed time: %.2f ms / Consumed memory: %.2f MB', $user->getId() ,$event->getDuration(), $event->getMemory() /1000 / 1000);
    ...
    */
    ...
    private function createUser(string $email,string $password ,bool $isAdmin): User 
    {
        ...
        $encodePassword = $this->encoder->hashPassword($user, $password);
